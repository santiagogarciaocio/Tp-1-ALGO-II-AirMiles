...
\Tipo{$\seq{Usuario}$}
    ...
    \requiere{n > 0}
    \aseguraLargo{
        (n >=|am.historialesTransacciones| \yluego res = claves(am.historialesTransacciones)) \lor_l \\
        ((\forall i \in $\mathbb{Z}$)(0 \geq i < n \yluego rankingMillasDisponibles(am.historialesTransacciones,res[i]) <= n)  \land_l \\
        |res| = n )                      
    }
\end{proc}
...
\Tipo{}
    ...
    \aseguraLargo{|am.historialesTransacciones| = |AM_0.historialesTransacciones| \land \\
        ...
        am.promociones = añadirPromocionSinTope(historialesVuelos,nombrePromocion,factor)
    }
\end{proc}
...
\Tipo{}
    ...
    \aseguraLargo{|am.historialesTransacciones| = |AM_0.historialesTransacciones| \land \\
        |am.historialesVuelos| = |AM_0.historialesVuelos| \land \\
        am.promociones = añadirPromocionConTope(historialesVuelos,nombrePromocion,factor,montoTope)
    }
\end{proc}
...
\Tipo{}
    ...
    \aseguraLargo{|am.historialesTransacciones| = |AM_0.historialesTransacciones| \land \\
        |am.historialesVuelos| = |AM_0.historialesVuelos| \land \\
        am.promociones = eliminarPromocion(historialesVuelos,nombrePromocion)
    }
\end{proc}
...
\Tipo{Usuario}
    ...
    \aseguraLargo{
        ...
        (\forall k \in $\mathbb{Z}$)(k \in am.historialesTransacciones \yluego totalGastadas(am.historialesTransacciones[k]) <= totalGastadas(am.historialesTransacciones[res]))
        %ver que onda el caso vacio (cuando recien se inicializa el TAD)
        }
\end{proc}
...
\Tipo{Usuario}
    ...
    \aseguraLargo{
        res \in am.historialesTransacciones \land \\
        (\forall k \in $\mathbb{Z}$)(k \in am.historialesTransacciones \yluego millasTotales(am.historialesTransacciones[k]) <= millasTotales(am.historialesTransacciones[res]))
    }
    %ver que onda el caso vacio (cuando recien se inicializa el TAD)
\end{proc}
...
\Tipo{$\seq{\tupla{UsuarioXUsuario}}$}
    ...
    \requiere{}
    \aseguraLargo{
        (|am.historialesTransacciones| < 2 \yluego res = $\tupla{()}$) \land \\
        (\forall i,j \in Z)((i,j \in am.historialesTransacciones \land_l i\neq j \land parejaTransferidores(am.historieslTransacciones,i,j))\yluego (i,j) \in res) \land \\
        (sinRepetidosNiAnagramas(res))
    }
\end{proc}

\pred {sinRepetidosNiAnagramas}{listaTuplaUsuarios:\seq{\tupla{UsuarioXUsuario}}}{
    (\forall i \in Z)(0 \geq i < |listaTuplaUsuarios| \yluego (\forall j \in Z)((i\neq j \land_l 0\geq j <|listaTuplaUsuarios|))\yluego ((listaTuplaUsuarios[i]_0 \neq listaTuplaUsuarios[j]_0 \land listaTuplaUsuarios[i]_1 \neq listaTuplaUsuarios[j]_1) \land (listaTuplaUsuarios[i]_0 \neq listaTuplaUsuarios[j]_1 \land listaTuplaUsuarios[i]_1 \neq listaTuplaUsuarios[j]_0)))
    }
...
\\
\aux{rankingMillasDisponibles}{historial:HistorialTransaccion, usuario:Usuario}{$\mathbb{Z}$}{ |historial| + 1 - $\sum_{\forall k \in historial} ifThenElse(historial[k] <= historial[usuario],1,0)$}
\\
